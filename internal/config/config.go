...
// UPDATED Créer Config qui est la structure principale qui mappe l'intégralité de la configuration de l'application.
...
func LoadConfig() (*Config, error) {
	// UPDATED Spécifie le chemin où Viper doit chercher les fichiers de config.
	// on cherche dans le dossier 'configs' relatif au répertoire d'exécution.
	viper.AddConfigPath("configs/")
	// UPDATED Spécifie le nom du fichier de config (sans l'extension).
	viper.SetConfigName("config")
	// UPDATED Spécifie le type de fichier de config.
	viper.SetConfigType("yaml")

	// UPDATED : Définir les valeurs par défaut pour toutes les options de configuration.
	...
	// UPDATED : Lire le fichier de configuration.

	// UPDATED 4: Démapper (unmarshal) la configuration lue (ou les valeurs par défaut) dans la structure Config.
	var cfg Config
