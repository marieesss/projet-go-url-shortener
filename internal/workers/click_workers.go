...
func StartClickWorkers(workerCount int, clickEventsChan <-chan models.ClickEvent, clickService *services.ClickService) {
	...
	for i := 0; i < workerCount; i++ {
		...
		go clickWorker(clickEventsChan, clickService)
	}
}
...
func clickWorker(clickEventsChan <-chan models.ClickEvent, clickService *services.ClickService) {
	for event := range clickEventsChan { // Boucle qui lit les événements du channel
		...
		err := clickService.RecordClick(&click)
		if err != nil {
			log.Printf(
				"ERROR: Failed to save for LinkID %d (UserAgent: %s, IP: %s): %v",
				event.LinkID,
				event.UserAgent,
				event.IPAddress,
				err,
